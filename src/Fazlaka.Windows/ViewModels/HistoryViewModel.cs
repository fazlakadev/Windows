...
public partial class HistoryViewModel : ObservableObject
{
    ...
    private async Task LoadAsync(CancellationToken token)
    {
        ...
        try
        {
            var result = await _api.GetHistoryAsync(token);
            ...
            {
                HasError = true;
                ErrorMessage = result.Error ?? "فشل تحميل سجل المشاهدات.";
            }
        }
    ...
    [RelayCommand]
    private async Task ClearHistoryAsync(CancellationToken token)
    {
        await _api.ClearHistoryAsync(token);
        Items.Clear();
    }
